...
def fit_swc(
    ...
    *,
    options: Optional[SWCFitOptions] = None,
) -> SWCModel:
    """
    ...
    Args:
        ...
        options: `SWCFitOptions` controlling sampling, radius strategy, and section probing.

    Returns:
        A populated SWC model (`swctools.SWCModel`).
    """
    if options is None:
        options = SWCFitOptions()
    ...
    for pl_index, pl in enumerate(pls.as_arrays()):
        ...
        for i in range(samples.shape[0]):
            ...
            if not reused:
                ...
                try:
                    swc.nodes[nid]["radius_strategy"] = radius_strategy
                    if inside_mesh is not None:
                        swc.nodes[nid]["inside_mesh"] = bool(inside_mesh)
                except Exception:
                    pass
            else:
                try:
    ...
    try:
        logger.info(
            ...
            int(swc.number_of_nodes()),
            int(swc.number_of_edges()),
            int(total_samples),
            int(used_section),
            int(used_fallback),
        )
