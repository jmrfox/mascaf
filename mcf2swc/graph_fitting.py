...
def fit_morphology(
    ...
    Returns:
        A populated MorphologyGraph with nodes (xyz + radius) and edges preserving cycles.
    """
    if options is None:
        options = FitOptions()
    ...
    for pl_index in sorted(edge_groups.keys()):
        ...
        for i in range(samples.shape[0]):
            ...
            try:
                ...
                # Convention: negative outside in trimesh; >= 0 means inside/on
            ...
            if qk in pos_index:
                ...
                if (
                    ...
                    if inside_mesh is not None:
                        graph.nodes[nid]["inside_mesh"] = bool(inside_mesh)
            ...
            # Add edge to previous node in polyline (creates cycles naturally)
            if prev_node is not None:
                # Add edge even if node was reused - this creates cycles when endpoints overlap
                if prev_node != nid:  # Avoid self-loops
                    graph.add_edge(prev_node, nid)
                if reused:
                    try:
                        logger.debug(
                            "pl=%d i=%d: reused existing node %d for overlapping position, edge added to close cycle",
                            int(pl_index),
                            int(i),
                            int(nid),
                        )
