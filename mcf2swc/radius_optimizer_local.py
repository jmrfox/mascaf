"""
Local segment-based radius optimization.
...
to the mesh surface. This ensures local geometric accuracy rather than just
global surface area/volume matching.
"""
...
class LocalOptimizerOptions:
    """
    Configuration for local segment-based radius optimization.
...
class LocalRadiusOptimizer:
    ...
    def __init__(
        ...
        *,
        ...
        self.mesh = mesh
        self.options = options if options is not None else LocalOptimizerOptions()

        # Build node index mapping
        self.node_to_idx = {nid: i for i, nid in enumerate(sorted(skeleton.nodes()))}
        ...
        self.initial_radii = np.array(
            [
                _get_node_radius(skeleton, self.idx_to_node[i])
        ...
        logger.info(
            "LocalRadiusOptimizer initialized: nodes=%d, edges=%d, n_long=%d, n_rad=%d",
