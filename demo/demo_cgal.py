# %% [markdown]
# # MASCAF Demo with CGAL Skeletonization
#
# This notebook walks through the full MASCAF pipeline:
#
# 1. Load a demo mesh and run CGAL skeletonization.
...
# %% [markdown]
# ## Load mesh and run CGAL skeletonization
#
# Requires the CGAL executables to be built. See the README for instructions.
...
skeleton_path = output_dir / f"{demo_model}.polylines.txt"
operator = CGALOperator(config=CGALConfig.from_overrides())
operator.skeletonize(mesh_path, skeleton_path, quality_speed_tradeoff=0.3, medially_centered_speed_tradeoff=1.0)
skeleton = SkeletonGraph.from_txt(str(skeleton_path))
print(f"Skeleton: {skeleton_path} ({skeleton.number_of_nodes()} nodes)")
